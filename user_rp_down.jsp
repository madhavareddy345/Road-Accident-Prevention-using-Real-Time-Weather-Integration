...
<h1 style="color: #E2B842;text-align: center;margin-top: 20px;padding-top : 50px;"> REPORT DETAILS</h1>
...
<table class="table">
  <thead>
    <tr>
      ...
      <th style="text-align:center;width:210px;">REPORT</th>
    <!-- <th style="text-align:center;width:210px;">REPORT ABOUT</th>  -->
    <!-- <th style="text-align:center;width:210px;">REGISTRATION STATUS</th>
  ...
  <%-- <%
		...
		<%		
				try{
		             ...
		             ResultSet rs = stmt.executeQuery("select * from uc where req_st= '"+sta+"' and email= '"+email+"' ");  
				  
				 		while(rs.next())
				  		{
				 			 em = rs.getString("email"); 
				 			  st = rs.getString("st");  
				 			  req_st = rs.getString("req_st");
				 			  lo = rs.getString("lo");
				 			  yor = rs.getString("yor");
				  			%> 
					<%--  <%
					      String type = "";
    ...
    <tr>
      ...
      <td><form action="fc_rp_download.jsp">
      <input type="hidden" name="email" value="<%=em %>" />
      <button class="btn btn-primary" >DOWNLOAD</button>
<%--       <a href="#" onclick="view()" data-email="<%= em %>" data-loc="<%= lo %>" data-yor="<%= yor %>"  style="margin-left: 20px;">VIEW</a>
 --%>      </form></td>
   <%--    <td ><%=rs.getString("ra")%></td>
      <td ><%=rs.getString("st")%></td>
       <td ><%=rs.getString("req_st")%></td>
       <%if((st.equalsIgnoreCase("accepted")) && (req_st.equalsIgnoreCase("pending"))) {%>
        <td><a href="#" onclick="view()" data-email="<%= em %>" data-loc="<%= lo %>" data-yor="<%= yor %>"  style="margin-left: 20px;">CLICK</a></td>
        <%} %>
        <%if(st.equalsIgnoreCase("rejected")) {%>
        <td><a href="#" onclick="view1()" style="margin-left: 20px;">CLICK</a></td>
        <%} %>
        <%if((st.equalsIgnoreCase("accepted")) && (req_st.equalsIgnoreCase("finished"))) {%>
        <td><a href="#" onclick="view2()" style="margin-left: 20px;">CLICK</a></td>
        <%} %> --%>
...
/* var module = document.getElementById("module").value;
    ...
    <%-- function back(){
    	var email = '<%=em%>';
    	window.location.replace("fc_view_user_req.jsp");
     ...
     function view(){
     	var email = document.querySelector('a[data-email]').getAttribute('data-email');
     	var loc = document.querySelector('a[data-loc]').getAttribute('data-loc');
     	var yor = document.querySelector('a[data-yor]').getAttribute('data-yor');
         /* window.location.href = 'Pdf_gen?email=' + email ; */
         window.location.replace("Pdf_gen?email="+email+ "&&loc=" + loc+ "&&yor=" +yor);
     	
     	
     } 
     function view1(){
      	/* var email = document.querySelector('a[data-email]').getAttribute('data-email');
          window.location.href = 'fc_view_user1_req.jsp?email=' + email; */
      	window.alert("Your Registration is pending");
