...
<table class="table">
  <thead>
    ...
    <th style="text-align:center;">YEAR OF REPORT</th>
     ...
     <th style="text-align:center;width:210px;">REPORT STATUS</th>
  ...
  <%-- <%
		...
		<%		
				try{
		             ...
		             ResultSet rs = stmt.executeQuery("select * from uc");  
    ...
    <tr>
     ...
     <%if (req_st.equalsIgnoreCase("finished")){ %>
      <td><button class="btn btn-primary" onclick="sendf()">SEND</button></td>
     <%} %>
     <%if(req_st.equalsIgnoreCase("pending")){ %>
     <td><button class="btn btn-primary" onclick="sendp()">SEND</button></td>
     <%} %>
    <%if(req_st.equalsIgnoreCase("Report sended")){ %>
     <td><button class="btn btn-primary" onclick="sends()">SEND</button></td>
     <%} %>
...
var email = '<%=em%>';
     ...
     function sendf(){
    	 ...
    	 window.location.replace("Ad_send_rp?email="+email);
    	 window.alert("Report Sended Sucessfully");
     }
     function sendp(){
    	 location.reload();
    	 window.alert("Still the process is ongoing");
     }
     function sends(){
    	 location.reload();
    	 window.alert("Already Report Sended");
     }
   /*  function head(){
