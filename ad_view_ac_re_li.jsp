...
<title>admin View user register status</title>
...
<h1 style="color: #E2B842;text-align: center;margin-top: 20px;padding-top : 50px;">CLIENT DETAILS</h1>
...
<%
 ...
 <select style="margin-left:0px;" class="btn btn-primary" id="dropdown" onchange="updateTable()">
					 <option value="null">SELECT</option>
					  <option value="OVERALL LIST">OVERALL LIST</option>
					 <option value="ACCEPTED LIST">ACCEPTED LIST</option>
                     <option value="REJECTED LIST">REJECTED LIST</option>
...
<table class="table">
  <thead>
    ...
    <!-- <th style="text-align:center;width:210px;">BUG TYPE</th>
    <th style="text-align:center;width:210px;">STATUS</th> -->
    <th style="text-align:center;width:210px;">VIEW MORE DETAILS</th>
      </tr>
  </thead>
  <tbody>
  <%-- <%
		...
		<%		
				try{
					if (table.equalsIgnoreCase("OVERALL LIST")){
					   qurey = "select * from uc" ;
					}
					if (table.equalsIgnoreCase("ACCEPTED LIST")){
						qurey = "select * from uc where st ='"+status1+"'" ;
						}
					if (table.equalsIgnoreCase("REJECTED LIST")){
						qurey = "select * from uc where st ='"+status2+"'" ;
						}
    ...
    <tr>
      ...
      <td><a href="#" onclick="view()" data-email="<%= em %>" style="margin-left: 20px;">CLICK</a></td>
...
<script>
 /* var module = document.getElementById("module").value; */
    ...
    function updateTable() {
        ...
        window.location.replace("ad_view_ac_re_li.jsp?table="+selectedValue);
    } 
    function back(){
    	<%-- var email = '<%=em%>'; --%>
     ...
     function view(){
        ...
        window.location.href = 'ad_view_us1.jsp?email=' + email;
